...
public class SongController : MonoBehaviour
{
	...
	List<float> noteSpawningTimes = new List<float>();
    ...
    void SendNotes()
	{
        if (onNotesReady != null)
        {
            onNotesReady(noteSpawningTimes);
        }
    }
    ...
    void Start()
	{
		...
		if (preProcessSamples)
		{
			...
			StartCoroutine(WaitForEnOfFrame(bgThread));
		}
	}

	IEnumerator WaitForEnOfFrame(Thread thread)
	{
		while(true)
		{
			if(!thread.IsAlive)
	...
	void Update()
	{
		...
		if (realTimeSamples)
		{
			...
			if(realTimeSpectralFluxAnalyzer.analyzeSpectrum(realTimeSpectrum, audioSource.time))
			{
				MidiManagers.noteInstanciator.NoteInstantioation(noteSpawningTimes);
            }
	...
	public void getFullSpectrumThreaded()
	{
		try
		{
			...
			for (int i = 0; i < iterations; i++)
			{
				...
				{
                    noteSpawningTimes.Add(curSongTime);
                    //Manager.noteInstanciator.NoteInstantioation(noteSpawningTimes);
                }
			}
